...
// PUT /api/v1/settings/card-hero-slides
// Manages the Bell-Italia-style card hero (3 cards per slide)
export const updateCardHeroSlides = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction,
) => {
  try {
    const { cardHeroSlides, heroDisplayType } = req.body;

    const updateData: any = {};

    if (cardHeroSlides !== undefined) {
      if (!Array.isArray(cardHeroSlides)) {
        return res
          .status(400)
          .json({ success: false, message: "cardHeroSlides must be an array" });
      }
      updateData.cardHeroSlides = cardHeroSlides;
    }

    // heroDisplayType: "classic" | "card" | "both"
    if (heroDisplayType !== undefined) {
      updateData.heroDisplayType = heroDisplayType;
    }
    ...
    res.status(200).json({
      success: true,
      message: "Card hero slides updated",
      data: { settings: updated },
    });
  } catch (error) {
    next(error);
