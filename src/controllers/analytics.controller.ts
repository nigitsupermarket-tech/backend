  ...
  try {
    ...
    if (isStaffOnly) {
      const [
        ...
        prisma.pOSOrder.count({
          where: { processedById: staffId, status: "COMPLETED" },
        }),
        prisma.pOSOrder.aggregate({
          where: { processedById: staffId, status: "COMPLETED" },
          _sum: { total: true },
        ...
        data: {
          ...
          staff: {
            ...
            allTime: {
              ...
              sales: allTimeSales._sum.total || 0,
            },
    ...
    if (req.user?.role === "SALES") {
      const [
        ...
        prisma.product.count({
          where: { isOnPromotion: true, status: "ACTIVE" },
        }),
        prisma.user
          .findMany({
            where: { role: { in: ["STAFF", "ADMIN", "SALES"] as any } },
      ...
      const staffStats = await Promise.all(
        staffSalesData.map(async (s) => {
          ...
          return {
            ...
            name: s.name,
            role: s.role,
            today: { orders: tAgg._count.id, sales: tAgg._sum.total || 0 },
            thisMonth: { orders: mAgg._count.id, sales: mAgg._sum.total || 0 },
          };
        }),
      );
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          posRevenue: {
            today: todayPOSSales._sum.total || 0,
        ...
        .findMany({
          where: {
            role: {
              in: (isManager
                ? ["MANAGER", "STAFF", "SALES"]
                : ["STAFF", "ADMIN", "SALES", "MANAGER"]) as any,
            },
          },
      ...
      staffList.map(async (staff) => {
        const [todayAgg, monthAgg, allTimeAgg, openSession] = await Promise.all(
          [
            ...
            prisma.pOSOrder.aggregate({
              where: { processedById: staff.id, status: "COMPLETED" },
              _sum: { total: true },
              _count: { id: true },
            }),
        ...
        return {
          ...
          allTime: {
            orders: allTimeAgg._count.id,
            sales: allTimeAgg._sum.total || 0,
          },
        };
      }),
    );
    ...
    const posMonthSales = await prisma.pOSOrder.aggregate({
      where: { status: "COMPLETED", createdAt: { gte: startOfMonth } },
      _sum: { total: true },
    });
    ...
    res.status(200).json({
      success: true,
      data: {
        viewType: isManager ? "manager" : "admin",
        orders: {
          total: totalOrders,
          today: todayOrders,
        ...
        pos: {
          ...
          salesToday: todayPOSSales._sum.total || 0,
        },
        promotions: { active: activePromotions },
        revenue: {
...
export const getRevenueChart = async (
  ...
  try {
    ...
    const [onlineOrders, posOrders] = await Promise.all([
      ...
      prisma.pOSOrder.findMany({
        where: {
          ...
          status: "COMPLETED",
          ...(isStaffOnly && { processedById: staffId }),
        },
        select: { total: true, createdAt: true },
      }),
    ]);
    ...
    const chartMap: Record<
      string,
      { date: string; online: number; pos: number; revenue: number }
    > = {};
    ...
    (onlineOrders as any[]).forEach((o) => {
      ...
      if (chartMap[key]) {
        chartMap[key].online += o.total;
        chartMap[key].revenue += o.total;
      }
    });
    posOrders.forEach((o) => {
      const key = new Date(o.createdAt).toISOString().split("T")[0];
      if (chartMap[key]) {
        chartMap[key].pos += o.total;
        chartMap[key].revenue += o.total;
      }
    });
    ...
    res
      .status(200)
      .json({ success: true, data: { chart: Object.values(chartMap) } });
  } catch (error) {
    next(error);
  }
};
...
export const getStaffStats = async (
  ...
  try {
    ...
    const stats = await Promise.all(
      staffList.map(async (staff) => {
        const [posAgg, cashS, cardS, transferS, recentOrders, sessions] =
          await Promise.all([
            prisma.pOSOrder.aggregate({
              where: {
                processedById: staff.id,
            ...
            prisma.pOSSession.findMany({
              where: { staffId: staff.id, openedAt: { gte: start, lte: end } },
        ...
        return {
          ...
          period: {
            totalOrders: posAgg._count.id,
            totalSales: posAgg._sum.total || 0,
            totalDiscount: posAgg._sum.discountAmount || 0,
            avgOrderValue:
              posAgg._count.id > 0
                ? (posAgg._sum.total || 0) / posAgg._count.id
                : 0,
          },
          byPaymentMethod: {
            ...
            transfer: {
              orders: transferS._count.id,
              total: transferS._sum.total || 0,
            },
          },
          recentOrders,
          sessions,
    ...
    res
      .status(200)
      .json({ success: true, data: { stats, period: { start, end } } });
  } catch (error) {
    next(error);
  }
};
